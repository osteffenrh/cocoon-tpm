...
fn main() {
    ...
    eprintln!(
        "ossl-bare-sys: integration target={:?}, config_file={:?}, \
               args={:?}, cppflags={:?}, cflags={:?}",
        integration_configure_target,
        integration_configure_config_file,
        integration_configure_args,
    ...
    let config_contents =
        std::fs::read_to_string(&config_h).unwrap_or_else(|e| panic!("cannot read {}: {e}", config_h.display()));
    let ec_disabled = config_contents.lines().any(|line| {
        let trimmed = line.trim().trim_start_matches('#').trim();
        trimmed == "define OPENSSL_NO_EC"
    });
    if ec_disabled {
        let disabled: Vec<&str> = config_contents.lines().filter(|l| l.contains("OPENSSL_NO_")).collect();
        panic!(
